...
let saveFrameButton;
let frameCount = 0;
let mouseClickWord = -1;

function getWords(words)
{
   let x = words.science.length
   console.log(x);
   let i=0;
   while(i<count)
   {
...
{
  let off = round(map(data.length,0,15,90,30));
  if(pmouseX>630 && pmouseX<width && pmouseY>50 && pmouseY<(w*(count+1)))
  {
    hideSolution();
    let i = round(map(pmouseY,50,off*(count+1),0,count-1));
    console.log(i);
    if(wordsFilled==true)
    {
        let r = random(180);
